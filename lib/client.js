...
Client.prototype.connect = function(cfg) {
  ...
  this.config.password = ((typeof cfg.password === 'string' || typeof cfg.password === "function")
                          ? cfg.password
                          : undefined);
  ...
  function tryNextAuth() {
    ...
    switch (curAuth) {
      case 'password':
        var pw = self.config.password;
        if(typeof pw === "function") {
          pw = pw();
          if(typeof pw !== "string") throw new Error("Password callback function does not return string");
        }
        stream.authPassword(self.config.username, pw);
      break;
      case 'publickey':
